...
// ========================
var wssServer = SIP_WSS_SERVER;           // eg: raspberrypi.local
var profileUserID = getDbItem("profileUserID", null);   // Internal reference ID. (DON'T CHANGE THIS!)
var profileUser = SIP_EXTENSION;       // eg: 100
var profileName = 'CIOC';       // eg: Keyla James
var WebSocketPort = SIP_PORT;   // eg: 444 | 4443
var ServerPath = SIP_PATH;         // eg: /ws
var SipUsername = SIP_USERNAME;       // eg: webrtc
var SipPassword = SIP_TOKEN;       // eg: webrtc
...
var userAgentStr = getDbItem("UserAgentStr", "LibreERP");   // Set this to whatever you want.
...
function ReceiveCall(session) {
    ...
    // Time Stamp
    window.postMessage( {status : 'incomming' , number : did , buddy : buddyObj})
    ...
    session.on('terminated', function(response, cause) {
        ...
        window.postMessage( {status : 'terminated' })

        // pradeep : send the they disconnected message
    ...
    if(CurrentCalls >= 1){
        ...
        rinnger.src = hostingPrefex + "static/audio/Tone_CallWaiting.mp3";
        ...
        rinnger.src = hostingPrefex + "static/audio/Ringtone_1.mp3";
        session.data.rinngerObj = rinnger;
    }
...
function AnswerAudioCall(buddyObj) {
    // pradeep
    // CloseWindow();

    // buddy
    buddy = buddyObj.identity
    console.log("in answer audio call");
...
function RejectCall(buddy) {
    ...
    var session = getSession(buddy.identity);
    // if (session == null) {
...
function wireupAudioSession(lineObj) {
    ...
    session.on('progress', function (response) {
        ...
        if(response.status_code == 100){
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.trying })

        } else if(response.status_code == 180 || response.status_code == 183){
            $(MessageObjId).html(lang.ringing);

            window.postMessage( {type : 'callUpdate' , status : lang.ringing })
            ...
            var soundFile = hostingPrefex + "static/audio/Tone_EarlyMedia-European.mp3";
            if(UserLocale().indexOf("us") > -1){
                soundFile = hostingPrefex + "static/audio/Tone_EarlyMedia-US.mp3";
            }
            if(UserLocale().indexOf("gb") > -1){
                soundFile = hostingPrefex + "static/audio/Tone_EarlyMedia-UK.mp3";
            }
            if(UserLocale().indexOf("au") > -1){
                soundFile = hostingPrefex + "static/audio/Tone_EarlyMedia-Australia.mp3";
            }
            if(UserLocale().indexOf("jp") > -1){
                soundFile = hostingPrefex + "static/audio/Tone_EarlyMedia-Japan.mp3";
            }
    ...
    session.on('trackAdded', function () {
        ...
        window.postMessage( {type : 'callUpdate' , status : 'trackAdded' })
    ...
    session.on('accepted', function (data) {
        ...
        session.data.callTimer = window.setInterval(function(){
            ...
            window.postMessage( {type : 'callUpdate' , status : 'timer' , time : formatShortDuration(duration.asSeconds()) })
        ...
        window.postMessage( {type : 'callUpdate' , status : 'accepted' })
    ...
    session.on('bye', function () {
        ...
        window.postMessage( {type : 'callUpdate' , status : lang.call_ended  })
    });
    session.on('terminated', function (message, cause) {
        console.log("Session terminated");
        window.postMessage( {type : 'callUpdate' , status : 'terminated'  })
    });
    session.on('reinvite', function (session) {
        console.log("Session reinvited!");
        window.postMessage( {type : 'callUpdate' , status : 'reinvite'  })
    });
    //dtmf
    session.on('directionChanged', function() {
        ...
        window.postMessage( {type : 'callUpdate' , status : 'directionChanged' , direction : direction })
...
function updateLineScroll(lineNum , lineObj) {
    RefreshLineActivity(lineNum , lineObj);
...
function AudioCall(lineObj, dialledNumber) {
    ...
    console.log(lineObj , dialledNumber);
    ...
    updateLineScroll(lineObj.LineNumber , lineObj);
...
function AttendedTransfer(lineNum){
    ...
    newSession.on('accepted', function (response) {
        ...
        CompleteTransferBtn.on('click', function(){
            var transferOptions  = {
                receiveResponse: function doReceiveResponse(response){
                    ...
                    window.postMessage( {type : 'callUpdate' , status : lang.attended_transfer_complete_accepted })
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.attended_transfer_complete })
        ...
        TerminateAttendedTransferBtn.on('click', function(){
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.attended_transfer_call_ended })
    ...
    newSession.on('trackAdded', function () {
      console.log("pradeep 2 : track added");
    ...
    newSession.on('rejected', function (response, cause) {
        ...
        window.postMessage( {type : 'callUpdate' , status : lang.attended_transfer_call_rejected })
    ...
    newSession.on('terminated', function (response, cause) {
        ...
        window.postMessage( {type : 'callUpdate' , status : lang.attended_transfer_call_terminated })
...
function ConferenceDail(lineNum){
    ...
    window.postMessage( {type : 'callUpdate' , status : lang.connecting })
    ...
    newSession.on('progress', function (response) {
        ...
        window.postMessage( {type : 'callUpdate' , status : lang.conference_call_started });
        ...
        CancelConferenceDialBtn.on('click', function(){
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.canference_call_cancelled });
            updateLineScroll(lineNum);
        });
    ...
    newSession.on('accepted', function (response) {
        ...
        JoinCallBtn.on('click', function(){
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.conference_call_in_progress });
        ...
        TerminateAttendedTransferBtn.on('click', function(){
            ...
            window.postMessage( {type : 'callUpdate' , status : lang.conference_call_ended });
            updateLineScroll(lineNum);
        });
        TerminateAttendedTransferBtn.show();
    ...
    newSession.on('trackAdded', function () {
        console.log("pradeep : track added");
        window.postMessage( {type : 'callUpdate' , status : 'trackAdded' });
        var pc = newSession.sessionDescriptionHandler.peerConnection;
    ...
    newSession.on('rejected', function (response, cause) {
        ...
        window.postMessage( {type : 'callUpdate' , status : lang.conference_call_rejected });
    ...
    newSession.on('terminated', function (response, cause) {
        ...
        // $rootScope.$broadcast("call", {type : 'statusUpdate', status : lang.conference_call_terminated });

        window.postMessage( {type : 'callUpdate' , status : lang.conference_call_terminated });
...
function cancelSession(lineObj) {
    if(lineObj == null || lineObj.SipSession == null) {
      lineObj = CURRENT_LINE;
    };
    if (lineObj) {
    ...
    window.postMessage( {type : 'callUpdate' , status : "canceled" });
}
function holdSession(lineObj) {
    if(lineObj == null || lineObj.SipSession == null) {
      lineObj = CURRENT_LINE;
    };
    ...
    window.postMessage( {type : 'callUpdate' , status : lang.call_on_hold });

    // updateLineScroll(lineNum);
}
function unholdSession(lineObj) {
    if(lineObj == null || lineObj.SipSession == null) {
      lineObj = CURRENT_LINE;
    };

    console.log("Taking call off hold: ");
    ...
    window.postMessage( {type : 'callUpdate' , status : lang.call_in_progress });
...
function RefreshLineActivity(lineNum , lineObj){
  // var lineObj = FindLineByNumber(lineNum);
    var lineObj = lineObj;
    if(lineObj == null || lineObj.SipSession == null) {
        return;
