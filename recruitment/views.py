...
class JobsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        print self.request.GET,'ksskdjfgsgjskdfj'
        if 'unit__city' in self.request.GET:
            email = self.request.GET['unit__city']
            emailFilter =  Jobs.objects.filter(unit__city=email)
            data = emailFilter
            print unit__city
            print data.distinct()
...
class JobApplicationViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend]
    filter_fields = ['job','status' ,'email' ,'mobile','firstname','rejectReason']
...
class JobsList(APIView):
    ...
    def get(self , request , format = None):
        ...
        if 'jobtype' in request.GET:
            querySet = Jobs.objects.filter(jobtype = str(request.GET['jobtype']))
        if 'city' in request.GET:
            print request.GET['city'],'oooooooooo'
            querySet = Jobs.objects.filter(unit__city__icontains = str(request.GET['city']))
        else:
            querySet = Jobs.objects.all()
        toReturn = list(querySet.values('pk','jobtype','department__dept_name','skill','maximumCTC','unit__name','role__name', 'unit__city'))
        print toReturn
...
class expanseReportHead(Flowable):
    ...
    def draw(self):
        ...
        pSrc = '''
        <font size=14>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<u>Joining Letter</u></font><br/><br/><br/>
        <font size=9>
...
class PageNumCanvas(canvas.Canvas):
    ...
    def drawLetterHeadFooter(self):
        ...
        try:
            companyName = userRecObj.designation.unit.name
        except:
            companyName = ''
        try:
            companyAddress = userRecObj.designation.unit.address + ' ' + userRecObj.designation.unit.city + ' ' +  userRecObj.designation.unit.state + ' ' + userRecObj.designation.unit.country + ' - ' +   userRecObj.designation.unit.pincode
        except:
            companyAddress = ''

        try:
            companyDetails = userRecObj.designation.unit.mobile + ' ' + userRecObj.designation.unit.email
        except:
            companyDetails = ''
        p = Paragraph(companyName, compNameStyle)
        p.wrapOn(self, 50 * mm, 10 * mm)
        p.drawOn(self, 85 * mm, 18 * mm)

        p1 = Paragraph(companyAddress, compNameStyle)
        p1.wrapOn(self, 200 * mm, 10 * mm)
        p1.drawOn(self, 15 * mm, 10 * mm)


        p2 = Paragraph(companyDetails, compNameStyle)
...
def genInvoice(response, contract, request):
    ...
    if contract.noticePeriodRecovery == 1:
        recovery = 'YES'
    else:
        recovery = 'NO'
    ...
    summryParaSrc = """
    <font size='10'>Dear <strong>%s %s, </strong></font> <br/> <br/>
    <font size='10'>I am well and wish you the same!</font> <br/> <br/>
    <font size='10'>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;Through this letter I would like to offer you the position of <strong>%s - %s</strong>. Starting from %s. Given below is your salary structure and job details.
    </font> <br/> <br/>
    <font size='10'><strong>Details</strong></font> <br/> <br/>
    <font size='10'>Position : %s - %s</font> <br/>
    <font size='10'>Reporting Date : %s</font> <br/>
    <font size='10'>Reporting Location : 4th Floor, Venkateshwara Heritage,<br/>
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp; Kudlu Hosa Road,<br/>
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;Opp Sai Purna Premium Apartment,<br/>
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;Sai Meadows, Kudlu,<br/>
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;Bengaluru, Karnataka 560068</font> <br/>
    <font size='10'>Weekly off : %s Sunday</font> <br/>
    <font size='10'>Notice period : %s Days</font> <br/>
    <font size='10'>Notice Period Recovery: %s</font> <br/>
    <font size='10'>Probation period : %s Days</font> <br/>
    <font size='10'>Notice Period During Probation: %s Days</font> <br/><br/>
    <font size='10'><strong>Compensation(monthly)</strong></font> <br/> <br/>
    <font size='10'>Basic : %s INR </font> <br/>
    <font size='10'>HRA : %s INR</font> <br/>
    <font size='10'>LTA : %s INR</font> <br/>
     <font size='10'>Special Allowance : %s INR</font> <br/>
     <font size='10'>AdHOC :  %s INR</font> <br/><br/>
     <font size='10'><strong>Others</strong></font> <br/> <br/>
    <font size='10'>Tax Slab : %s &#37;</font> <br/>
    <font size='10'>Medical Insurance Amount :  %s INR</font> <br/><br/>
    <font size='10'><strong>Leaves</strong></font> <br/> <br/>
    <font size='10'>Annual :  %s Days </font> <br/>
    <font size='10'>Medical : %s Days</font> <br/>
    <font size='10'>AdHOC : %s Days</font> <br/><br/>
    <font size='10'>I am looking forward to have a great working experience with you.</font> <br/> <br/>
...
class SendCallLetter(APIView):
    ...
    def post(self, request, format=None):
        contactData=[]
        response = HttpResponse(content_type='application/pdf')
        o = JobApplication.objects.get(pk=request.data['value'])
        response['Content-Disposition'] = 'attachment; filename="Call_letter%s_%s_%s.pdf"' % (
             o.job.pk, datetime.datetime.now(pytz.timezone('Asia/Kolkata')).year, o.pk)
        genInvoice(response, o, request)
        f = open(os.path.join(globalSettings.BASE_DIR, 'media_root/Call_letter%s%s_%s.pdf' %
        ...
        msgBody = "Hi " + request.data['first_name'] + " " + request.data['last_name'] + ",\n\n\t\t We are glad to inform you that you are been selected for the post of " + request.data['job'] +" - " + request.data['jobType'] + " Please find the below attachment for more details"
        email = request.data['emailID']
        contactData.append(str(email))
        msg = EmailMessage(email_subject, msgBody,  to= contactData )
...
class SaveDetails(APIView):
    renderer_classes = (JSONRenderer,)
    def post(self, request, format=None):
        user = User.objects.create(username = request.data['username'],first_name = request.data['firstname'],last_name = request.data['lastname'],email=request.data['email'])
        prof = profile.objects.get(pk=user.profile.pk)
        prof.email = request.data['newAddedEmail']
        prof.mobile =  request.data['mobile']
        prof.save()
        pay = payroll.objects.create(user = user)
        pay.hra = request.data['hra']
        pay.special = request.data['special']
        pay.lta = request.data['lta']
        pay.basic = request.data['basic']
        pay.taxSlab = request.data['taxSlab']
        pay.adHoc = request.data['adHoc']
        pay.amount = request.data['amount']
        pay.noticePeriodRecovery = request.data['noticePeriodRecovery']
        pay.al = request.data['al']
        pay.ml = request.data['ml']
        pay.adHocLeaves = request.data['adHocLeaves']
        pay.joiningDate = request.data['joiningDate']
        pay.save()
        des = designation.objects.create(user = user)
        unitDetails = Unit.objects.get(pk = request.data['job']['unit']['pk'])
        des.unit = unitDetails
        roleDetails = Role.objects.get(pk = request.data['job']['role']['pk'])
        des.role = roleDetails
        des.save()
        return Response({}, status = status.HTTP_200_OK)

class DashboardDataRecruitAPIView(APIView):
    renderer_classes = (JSONRenderer,)
    def get(self, request, format=None):
        toRet = {}
        jobs = Jobs.objects.filter(status='Active')
        rejectListT = JobApplication.objects.filter(rejectReason='Techical')
        rejectListS = JobApplication.objects.filter(rejectReason='Salary')
        rejectListE = JobApplication.objects.filter(rejectReason='Experience')
        rejectListH = JobApplication.objects.filter(rejectReason='HR')
        rejectListO = JobApplication.objects.filter(rejectReason='Others')
        onboard = JobApplication.objects.filter(status='Onboarding')
        allApply = JobApplication.objects.all()
        interview = Interview.objects.all()
        toRet['totalApply'] = allApply.count()
        toRet['jobCount'] = jobs.count()
        toRet['onboardCount']=onboard.count()
        toRet['interviewCount']=interview.count()
        toRet['techCount']=rejectListT.count()
        toRet['salCount']=rejectListS.count()
        toRet['expCount']=rejectListE.count()
        toRet['hrCount']=rejectListH.count()
        toRet['othCount']=rejectListO.count()
...
class JobUploadAPIView(APIView):
    ...
    def post(self, request, format=None):
        ...
        jobObj = Jobs.objects.get(pk = int(request.data['job']))
